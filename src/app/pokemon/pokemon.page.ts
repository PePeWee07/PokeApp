...
export class PokemonPage implements OnInit {
  constructor(private pokeApi: PokeApiServiceService, private alertController: AlertController) {}
    ...
    this.pokeApi.getPokemonList(this.limit, this.offset).subscribe(
      (data: any) => {
        ...
        const requests = this.pokemonLink.map((pokemon: Result) => {
          return this.pokeApi.getPokemon(pokemon.url);
        });
...
//color de borde dependiendo del tipo de pokemon con la clases
  pokeClass(type: any){
  ...
  //iamgen svg dependiendo del tipo de pokemon
  pokeSVG(type: any){
  ...
  //color de borde dependiendo del tipo de pokemon
  borderCarPokemon(types: any){
    const typeColors: Record<string, string> = {
      grass: "#5FBD58",
      fire: "#F08030",
      water: "#6890F0",
